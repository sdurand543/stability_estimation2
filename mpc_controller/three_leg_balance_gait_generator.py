...
class OpenloopGaitGenerator(gait_generator.GaitGenerator):
  ...
  def update(self, current_time):
    ...
    for leg_id in range(self._robot.num_legs):
      ...
      phase_in_full_cycle = current_time/self.lift_time

      if phase_in_full_cycle > 1:
        phase_in_full_cycle = 1
      elif phase_in_full_cycle<0:
        phase_in_full_cycle = 0

      self._normalized_phase[0], self._normalized_phase[3] = (phase_in_full_cycle, )*2

      self._leg_state[0], self._leg_state[3] = (gait_generator.LegState.SWING, )*2
      self._leg_state[1], self._leg_state[2] = (gait_generator.LegState.STANCE,) * 2
