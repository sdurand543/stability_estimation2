...
def main(argv):
  ...
  block2=p.createMultiBody(baseMass=0,baseCollisionShapeIndex = sh_colBox,
                          basePosition = [1.22,0.6,-0.05],baseOrientation=[0.0,0.0,0.0,1])
  ...
  joint_angles_curr = actuate_joint_angles(joint_angles_curr, joint_angles_end, 200)
  ...
  joint_angles_curr = actuate_joint_angles(joint_angles_curr, joint_angles_end, 2000)
  ...
  trigger_foot_force = robot.GetFootForce()['10']
  START_FORCE_OFFSET = trigger_foot_force
  MAX_FORCE = START_FORCE_OFFSET
  FOOT_SENSOR_NOISE_THRESHOLD = START_FORCE_OFFSET + 2
  DATA_COLLECTION_STOP_THRESHOLD = FOOT_SENSOR_NOISE_THRESHOLD + 12
  SKIP_TEN = 0
  stable = False
  BREAK = False
  ...
  for curr_step in range(num_steps):
      ...
      if trigger_foot_force > MAX_FORCE and curr_step > SKIP_TEN:
        MAX_FORCE = trigger_foot_force

      if trigger_foot_force > DATA_COLLECTION_STOP_THRESHOLD - 5 and curr_step > SKIP_TEN:
        if stable:
          pass
        else:
          p.resetBasePositionAndOrientation(block2, [1.22,0.6,-0.08],[0.0,0.0,0.0,1])
          BREAK = True

      if trigger_foot_force > DATA_COLLECTION_STOP_THRESHOLD or trigger_foot_force < MAX_FORCE*0.35:
        data_collection = False
  ...
  while current_time - start_time < FLAGS.max_time_secs:
    ...
    if current_time > 0.5 and BREAK == False:
      lin_speed = np.array([0.1,0,0])
    if current_time > 0.5 and BREAK == True:
